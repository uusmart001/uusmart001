...
演示 Bilibili 动态页监听：通过 `add_dynamic_page_watch` 同时订阅多个 UP 主的新动态，一次 API 调用覆盖所有订阅用户，比逐个 `add_dynamic_watch` 更高效。

- `on_load` 中批量添加 UID，插件自动关注目标 UP 主（已关注则跳过）
- `on_dynamic_new` 接收 `BiliDynamicEvent`，读取动态类型与文本内容
- `on_unload` 中清理所有监听
