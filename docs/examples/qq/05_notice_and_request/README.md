...
NcatBot 中**如何处理 QQ 通知（Notice）和请求（Request）事件**。通过 `@registrar.qq.on_group_increase()` 等细分装饰器，你可以为群成员变动、消息撤回、戳一戳、好友/群请求等事件注册专属处理器，同时用 `@registrar.qq.on_notice()` 做兜底。

## 你将学到

- `on_group_increase` — 群成员增加，发送欢迎消息
- `on_group_decrease` — 群成员减少（退群/被踢），记录日志
- `on_group_recall` — 消息撤回处理
- `on_poke` — 戳一戳事件，判断目标后回戳
- `on_group_msg_emoji_like` — 群消息表情回应
- `on_friend_request` — 好友请求处理（approve / reject）
- `on_group_request` — 加群申请 / 邀请处理
- `on_notice` — 通用兜底，捕获所有未被细分处理器匹配的通知

## 前置知识

- [01_event_registration](../01_event_registration/) — 了解 `on_notice` / `on_request` 基础注册方式
...
| 装饰器 | 事件类型 | 触发场景 | 关键字段 |
|--------|---------|---------|---------|
| `on_group_increase()` | `GroupIncreaseEvent` | 新成员入群 | `user_id`, `operator_id`, `sub_type` |
| `on_group_decrease()` | `GroupDecreaseEvent` | 成员退群/被踢 | `user_id`, `operator_id`, `sub_type` |
| `on_group_recall()` | `GroupRecallEvent` | 群消息撤回 | `message_id`, `user_id`, `operator_id` |
| `on_poke()` | `PokeNotifyEvent` | 戳一戳 | `user_id`, `target_id`, `group_id` |
| `on_group_msg_emoji_like()` | `GroupMsgEmojiLikeEvent` | 表情回应 | `message_id`, `likes` |

### Request 事件处理器

| 装饰器 | 事件类型 | 触发场景 | 关键字段 |
|--------|---------|---------|---------|
| `on_friend_request()` | `FriendRequestEvent` | 好友申请 | `user_id`, `comment` |
| `on_group_request()` | `GroupRequestEvent` | 加群申请/邀请 | `user_id`, `sub_type`, `comment`, `group_id` |

### 细分 vs 兜底

```text
事件到达
  ├─ 匹配 on_group_increase? → on_member_join()
  ├─ 匹配 on_group_decrease? → on_member_leave()
  ├─ 匹配 on_group_recall?   → on_recall()
  ├─ 匹配 on_poke?           → on_poke()
  ├─ 匹配 on_group_msg_emoji_like? → on_emoji_like()
  └─ 都不匹配?               → on_any_notice()  ← 兜底
```

细分装饰器和 `on_notice()` 并不互斥——**两者都会被调用**。`on_notice()` 适合做统一日志或监控，细分装饰器负责具体业务逻辑。
...
- 只在群聊中（`event.group_id` 存在）且 Bot 被戳时才回戳
...
# 群请求 — 拒绝并说明理由
await event.reject(reason="暂不接受加群申请")
~~~

`FriendRequestEvent` 和 `GroupRequestEvent` 都继承了 `approve()` / `reject()` 方法，调用方式完全一致。

## 运行方式

1. 将 `05_notice_and_request/` 文件夹复制到项目的 `plugins/` 目录
2. 启动 Bot
3. 触发对应事件（入群、退群、撤回消息、戳一戳、发送好友/群请求）查看日志和响应

## 延伸阅读

- [01_event_registration](../01_event_registration/) — 事件注册基础
- Reference 文档：QQ 事件类型完整列表
- 更多 Notice 类型：`on_group_admin`（管理员变动）、`on_group_ban`（禁言）、`on_friend_add`（好友已添加）
